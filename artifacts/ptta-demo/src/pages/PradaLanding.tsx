...
// Hero "video" is implemented as a slow Ken-Burns cross-fade of cinematic
// stills — gives the same lived feel without depending on the .mp4 (which
// was iCloud-evicted and is unrecoverable).
const HERO_FRAMES = [
  `${BASE}/paintings/st-nikolai.jpg`,
  `${BASE}/paintings/persistence-of-memory.jpeg`,
  `${BASE}/paintings/the-scream.jpg`,
];

const PRODUCT_GRID = [
  { img: `${BASE}/paintings/st-nikolai.jpg`,             caption: "St. Nikolai",            href: "/painting-to-model" },
  { img: `${BASE}/paintings/persistence-of-memory.jpeg`, caption: "Persistence of Memory",  href: "/painting-to-model" },
  { img: `${BASE}/paintings/eiffel-tower.webp`,          caption: "Eiffel Tower",           href: "/painting-to-model" },
  { img: `${BASE}/paintings/the-scream.jpg`,             caption: "The Scream",             href: "/painting-to-model" },
];

const CATEGORY_TILES = [
  { img: `${BASE}/printed/persistence-of-memory.png`, caption: "Tastmodelle",       href: "/demo-hub" },
  { img: `${BASE}/images/audio-guide.jpg`,            caption: "Audio-Guide",       href: "/audio-guide" },
  { img: `${BASE}/artists/leonardo.webp`,             caption: "Künstler-Persona",  href: "/artist-persona" },
  { img: `${BASE}/images/future-of-art.jpg`,          caption: "Zukunft",           href: "/future-features" },
];

const EDITORIAL_PAIR = {
  thumb: `${BASE}/paintings/eiffel-tower.webp`,
  largeImg: `${BASE}/printed/renders/mona-lisa-02.png`,
...
export default function PradaLanding() {
  ...
  // Cycle hero frames every 5s for the "video-like" feel.
  useEffect(() => {
    const id = window.setInterval(
      () => setHeroFrame((i) => (i + 1) % HERO_FRAMES.length),
      5000,
    );
    ...
    <div className="prada-root min-h-screen bg-white text-black">
      ...
      >
        <div className="grid grid-cols-3 items-center px-6 md:px-10 py-4">
          ...
          {/* Wordmark — Cormorant Garamond, elegant serif. Full brand name in
              the header rather than the abbreviated PTTA. */}
          <a
            href="/"
            onClick={go("/")}
            className="text-center text-black leading-none whitespace-nowrap"
            aria-label="Please Touch This Art — home"
          >
            <span className="prada-wordmark block">Please Touch This Art</span>
          </a>
      ...
      {/* HERO — full-bleed cinematic cross-fade with slow Ken-Burns pan.
          Replaces the originally planned <video>, which was iCloud-evicted. */}
      <section className="relative w-full" aria-label="Hero">
        <div className="prada-hero-stage relative w-full aspect-[16/9] md:aspect-[21/9] overflow-hidden bg-stone-200">
          {HERO_FRAMES.map((src, i) => (
            <div
              key={src}
              className="prada-hero-frame absolute inset-0"
              style={{ opacity: heroFrame === i ? 1 : 0 }}
              aria-hidden={heroFrame !== i}
            >
              <img
                src={src}
                alt=""
                className="absolute inset-0 w-full h-full object-cover prada-hero-img"
              />
            </div>
          ))}
          <div
            ...
            style={{
              background:
                "linear-gradient(to top, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.08) 40%, transparent 75%)",
            }}
          />
          <motion.div
      ...
      <section className="bg-[#f6f5f3] py-16 md:py-24" aria-label="Mission editorial">
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center px-6 md:px-10">
          ...
          <div className="prada-hero-stage relative aspect-[5/6] overflow-hidden bg-stone-100">
            <img
              src={EDITORIAL_PAIR.largeImg}
              alt=""
              loading="lazy"
              className="absolute inset-0 w-full h-full object-cover prada-hero-img"
            />
      ...
      <footer className="border-t border-black/10 py-14 md:py-20 px-6 md:px-10 text-center bg-white" role="contentinfo">
        <p className="prada-wordmark mb-3" style={{ fontSize: 28 }}>Please Touch This Art</p>
